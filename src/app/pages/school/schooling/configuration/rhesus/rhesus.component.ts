...
import { RhesusService } from '../../../services/rhesus.service';
...
export class RhesusComponent {
  ...
  dataSource!: MatTableDataSource<Rhesus>;
  ...
  getList(){
    this._rhesusService.getRhesusList().subscribe({
      next: (res: any) => {
        this.rhesus = res['hydra:member'];
        console.log(this.rhesus);

        this.dataSource = new MatTableDataSource(this.rhesus)
  ...
  openAddForm() {
    const dialogRef = this._modalService.open(RhesusAddEditComponent, {
      data: { title: 'rhesus Modal' },
  ...
  delete(id:number){
    Swal.fire({
      ...
      if (result.isConfirmed) {
        this._rhesusService.delete(id).subscribe({
          next:(res) => {
            Swal.fire(
              'Deleted!',
