...
export class ClassCategoryComponent {
  ...
  dataSource!: MatTableDataSource<ClassCategory>;
  ...
  getList(){
    this._classCategoryService.getList().subscribe({
      next: (res: any) => {
        this.classcategory = res['hydra:member'];
        console.log(this.classcategory);

        this.dataSource = new MatTableDataSource(this.classcategory)
  ...
  openAddForm() {
    const dialogRef = this._modalService.open(ClassCategoryAddEditComponent, {
      data: { title: 'classcategory Modal' },
  ...
  delete(id:number){
    Swal.fire({
      ...
      if (result.isConfirmed) {
        this._classCategoryService.delete(id).subscribe({
          next:(res) => {
            Swal.fire(
              'Deleted!',
