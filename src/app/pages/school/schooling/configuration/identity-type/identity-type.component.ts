...
export class IdentityTypeComponent {
  ...
  public indentityType!: IdentityType[];
  dataSource!: MatTableDataSource<IdentityType>;
  ...
  getList(){
    this._identitytypeService.getList().subscribe({
      next: (res: any) => {
        this.indentityType = res['hydra:member'];
        console.log(this.indentityType);

        this.dataSource = new MatTableDataSource(this.indentityType)
  ...
  openAddForm() {
    const dialogRef = this._modalService.open(IdentityTypeAddEditComponent, {
  ...
  delete(id:number){
    Swal.fire({
      ...
      if (result.isConfirmed) {
        this._identitytypeService.delete(id).subscribe({
          next:(res) => {
            Swal.fire(
              'Deleted!',
