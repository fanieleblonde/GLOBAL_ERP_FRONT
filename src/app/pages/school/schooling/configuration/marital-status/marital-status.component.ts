...
export class MaritalStatusComponent {
  ...
  dataSource!: MatTableDataSource<MaritalStatus>;
  ...
  getList(){
    this._maritalstatusService.getList().subscribe({
      next: (res: any) => {
        this.maritalStatus = res['hydra:member'];
        console.log(this.maritalStatus);

        this.dataSource = new MatTableDataSource(this.maritalStatus)
  ...
  openAddForm() {
    const dialogRef = this._modalService.open(MaritalStatusAddEditComponent, {
  ...
  delete(id:number){
    Swal.fire({
      ...
      if (result.isConfirmed) {
        this._maritalstatusService.delete(id).subscribe({
          next:(res) => {
            Swal.fire(
              'Deleted!',
