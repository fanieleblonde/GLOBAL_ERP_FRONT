...
import { ReligionAddEditComponent } from './religion-add-edit/religion-add-edit.component';
...
export class ReligionComponent {
  ...
  dataSource!: MatTableDataSource<Religion>;
  ...
  getList(){
    this._religionService.getReligionList().subscribe({
      next: (res: any) => {
        this.religion = res['hydra:member'];
        console.log(this.religion);

        this.dataSource = new MatTableDataSource(this.religion)
  ...
  openAddForm() {
    const dialogRef = this._modalService.open(ReligionAddEditComponent, {
  ...
  delete(id:number){
    Swal.fire({
      ...
      if (result.isConfirmed) {
        this._religionService.delete(id).subscribe({
          next:(res) => {
            Swal.fire(
              'Deleted!',
