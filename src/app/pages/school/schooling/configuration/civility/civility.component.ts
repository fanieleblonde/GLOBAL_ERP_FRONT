...
export class CivilityComponent {
  ...
  dataSource!: MatTableDataSource<Civility>;
  ...
  getList(){
    this._civilityService.getList().subscribe({
      next: (res: any) => {
        this.civility = res['hydra:member'];
        console.log(this.civility);

        this.dataSource = new MatTableDataSource(this.civility)
  ...
  openAddForm() {
    const dialogRef = this._modalService.open(CivilityAddEditComponent, {
  ...
  delete(id:number){
    Swal.fire({
      ...
      if (result.isConfirmed) {
        this._civilityService.delete(id).subscribe({
          next:(res) => {
            Swal.fire(
              'Deleted!',
