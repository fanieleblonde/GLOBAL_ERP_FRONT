...
import { ModeAddEditComponent } from './mode-add-edit/mode-add-edit.component';
...
export class ModeComponent {
  ...
  getModeList(){
    this._modeService.getList().subscribe({
      next: (res: any) => {
        this.mode = res['hydra:member'];
        console.log(this.mode);

        this.dataSource = new MatTableDataSource(this.mode)
  ...
  openAddForm() {
    const dialogRef = this._modalService.open(ModeAddEditComponent, {
      data: { title: 'mode Modal' },
  ...
  delete(id:number){
    Swal.fire({
      ...
      if (result.isConfirmed) {
        this._modeService.delete(id).subscribe({
          next:(res) => {
            Swal.fire(
              'Deleted!',
