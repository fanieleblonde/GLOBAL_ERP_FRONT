...
import {SkillTypeAddEditComponent} from "./skill-type-add-edit/skill-type-add-edit.component";
...
export class SkillTypeComponent {
  ...
  getWorkLocationList(){
    this._skillTypeService.getList().subscribe({
      next: (res: any) => {
        this.skillType = res['hydra:member'];
        console.log(this.skillType);

        this.dataSource = new MatTableDataSource(this.skillType)
  ...
  openAddCampusForm() {
    const dialogRef = this._modalService.open(SkillTypeAddEditComponent, {
      data: { title: 'skillType Modal' },
  ...
  delete(id:number){
    Swal.fire({
      ...
      if (result.isConfirmed) {
        this._skillTypeService.delete(id).subscribe({
          next:(res) => {
            Swal.fire(
              'Deleted!',
